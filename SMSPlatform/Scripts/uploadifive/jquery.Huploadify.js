(function ($) {
    $.fn.Huploadify = function (opts) {
        ...
        var defaults = {
            ...
            removeTimeout: 1,//上传完成后进度条的消失时间 秒
            ...
            //        , removeCompleted:null//是否删除传输完成的项
        ...
        this.each(function () {
            ...
            inputStr += ' accept="';
            inputStr += $(getFileTypes(option.fileTypeExts)).map(function (index, element) {
                return $.MIMEType ? $.MIMEType(element) : element;
            }).toArray().join(",");
            inputStr += '"/>';
                ...
                funDeleteFile: function (index) {
                    for (var i = 0, len = this.fileFilter.length; i < len; i++) {
                        ...
                        if (file.index == index) {
                            ...
                            _this.find('#fileupload_' + instanceNumber + '_' + index).fadeOut(100);
...
//                                    debugger;
                                    if (xhr.responseText) {
                                        ...
                                        thisfile.find('.uploadify-progress-bar').css('backgroundColor', 'red');
                                        option.showUploadedPercent &&
                                            thisfile.find('.up_percent').text(xhr.responseText);
                                    } else {
                                    ...
                                    thisfile.find('.uploadify-progress-bar').css('backgroundColor', 'red');
                                    option.showUploadedPercent && thisfile.find('.up_percent').text(xhr.responseText);
                                }
                        ...
                        if (option.onUploadStart && option.onUploadStart(this)) {
                            // 开始上传
                            ...
                            _this.find('.up_percent').text('用户取消上传');
                        }
